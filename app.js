...
const i18n = {
  pt: {
    ...
    closeHelp: "Fechar",
    cssNotSaved: "Edições aqui valem só nesta sessão: ao recarregar, o CSS do tema volta ao original.",
    confirmOk: "OK",
    cancel: "Cancelar",
  ...
  en: {
    ...
    closeHelp: "Close",
    cssNotSaved: "Edits here last only for this session: on reload, the theme CSS returns to the original.",
    confirmOk: "OK",
    cancel: "Cancel",
...
function loadThemeCss(){$('custom-css').value=currentTheme().css;}
...
const css=$('custom-css').value.replace(/<\/style/gi,'<\\/style');
...
if($('import-css'))$('import-css').onclick=()=>$('css-files').click();$('css-files').onchange=async e=>{const files=[...e.target.files];let count=0,errors=[];for(const file of files){try{if(!/\.css$/i.test(file.name))throw Error(file.name+': '+t('selectCss'));if(file.size>1024*1024)throw Error(file.name+': '+t('limitCss'));const css=await file.text();if(!css.trim())throw Error(file.name+': '+t('emptyFile'));const id='custom:'+(crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+'-'+Math.random().toString(36).slice(2));const same=themes.filter(x=>x.name===file.name||x.name.startsWith(file.name+' (')).length;const theme={id,name:file.name+(same?' ('+(same+1)+')':''),css,custom:true,group:t('yourThemes')};imported.push(theme);themes.push(theme);activeId=id;count++;}catch(err){errors.push(err.message);}}updateOptions();loadThemeCss();render();save();toast([count?`${count} ${count===1?t('themeAdded'):t('themesAdded')}.`:'',...errors,!storageAvailable?t('storageFull'):''].filter(Boolean).join(' '));e.target.value='';};
if($('remove-css'))$('remove-css').onclick=async()=>{const x=currentTheme();if(!x.custom||!await ask(t('removeThemeBrowser', x.name)))return;imported=imported.filter(y=>y.id!==activeId);themes=[...window.BUILTIN_THEMES,...imported];activeId='sindresorhus/github-markdown-light.css';updateOptions();loadThemeCss();render();save();toast(t('themeRemoved'));};

if($('clear-imported-css'))$('clear-imported-css').onclick=async()=>{if(!imported.length){toast(t('noImportedCss'));return;}if(!await ask(t('removeAllThemes')))return;imported=[];themes=[...window.BUILTIN_THEMES];activeId='sindresorhus/github-markdown-light.css';updateOptions();loadThemeCss();render();save();toast(t('allThemesRemoved'));};
...
register({name:'set_document_theme',description:'Select a CSS theme, update the document preview and save this local preference. Does not export or change the Markdown text.',inputSchema:{type:'object',properties:{themeId:{type:'string'}},required:['themeId'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:true},async execute(input){if(!input||typeof input.themeId!=='string'||Object.keys(input).some(k=>k!=='themeId')||!themes.some(x=>x.id===input.themeId))throw Error(t('invalidTheme'));activeId=input.themeId;$('theme').value=activeId;loadThemeCss();await render();save();return{selected:activeId,name:currentTheme().name};}});
}
})();
